...
<div class="warning-msg"><?php echo __('Please install Recommended Plugins for demo to be imported completely.<br /> And Make sure you download demo in a fresh install. <br /> Your Old Data might be deleted.','wp-store');?></div>
<?php
wp_enqueue_style( 'plugin-install' );
wp_enqueue_script( 'plugin-install' );
wp_enqueue_script( 'updates' );
$wp_store_req_plugins = $this->wp_store_req_plugins;

foreach($wp_store_req_plugins as $slug=>$plugin) :
	if($plugin['bundled'] == false) {
		?>
		<div class="action-tab warning">
			<h3><?php printf( esc_html__("Install : %s Plugin", 'wp-store'), $plugin['name'] ); ?></h3>
			<p><?php echo esc_html__('Please check the plugins folder inside theme and upload the zip of plugins from plugin uploader.','wp-store');?></p>
		</div>
		<?php
	} else {
		...
		switch($status['needs']) {
			...
			$label = esc_html__('Install and Activate', 'wp-store');
			$link = $plugin['location'];
			break;

			case 'deactivate' :
			$btn_class = 'button';
			$label = esc_html__('Deactivate', 'wp-store');
			$link = admin_url('plugins.php');
			break;

			case 'activate' :
			$btn_class = 'activate-offline button button-primary';
			$label = esc_html__('Activate', 'wp-store');
			$link = $plugin['location'];
			break;
		}
		?>
